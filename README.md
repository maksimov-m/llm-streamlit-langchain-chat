...
1. **Название модели** — введите LLM, с которой планируете работать (например `llama-3-8b-instruct-8k`).
...
4. **API ключ** — вставьте ваш секретный ключ.
...
соответственно: model, model_provider, base_url, api_key.
...
Приложение использует переменные окружения (через `app/.env`):
